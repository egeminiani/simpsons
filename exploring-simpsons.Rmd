...
The Simpsons holds the title of the world's longest-running animated sitcom. It was created by the American writer Matt Groening in 1989 and is nowadays in its 31st season. The show gives us a satirical depiction of the working-class life of the Simpson family, which mainly consists of Homer, Marge, Bart, Lisa, and the little Maggie. 
...
* `guests` data set: we exclude the Movie season entries and create a logical variable `self`, which is equal to `TRUE` if a given guest star has played themselves in a particular episode, and `FALSE` if they have instead voiced a regular character. We additionally fix some typos on the character names.
...
Let us join `dialogues` with information contained in `characters` and `episodes`. This allows us to know the gender of the most talkative characters and the season of each episode. Notice that `dialogues` terminates at episode 568 (episode 16 of season 26).
...
Let us also join `episodes` with information from `guests`. This will be useful to get, for instance, the guest star names for each episode (if any), and the role they played. Notice that  `episodes` terminates at episode 596 (last episode of season 27), whereas `guests` at episode 662 (episode 23 of season 30). 
...
Let us look at the distribution of the total number of words of some of the leading characters throughout the seasons. We focus on the four most talkative ones, namely Homer, Marge, Lisa, and Bart Simpson. 
...
Let us have a look at the ratings, the number of votes, and the TV views across episodes and averaged over seasons.
...
The correlation matrix of `episodes` certainly shows a strong inverse relationship between the episode number and the ratings, the votes, and the views. This indicates that more recent seasons are generally affected by a decrease in those performance indicators.
...
It would be interesting to see whether the number of guest appearances over time. In the show's early years, most guest stars have voiced original characters, but as the show has continued, the number of those appearing as themselves has increased, especially throughout seasons 12 to 19. In more recent seasons, the gap between the two seems to have become more pronounced.
...
How many lines are generally reserved for guest stars per episode? The guest stars with the most lines per episode are the ones voicing a role like a narrator or an announcer and are usually not playing themselves. The only exception of a guest star playing themselves and having a fair amount of lines is Lady Gaga.
...
## Word and document frequency {.tabset .tabset-pills}
...
```{r arrr, fig.height=7, out.width="500"}
role.specificity %>%
...
Let us now have a look at some word clouds, which gives us an idea of the most recurrent words throughout the seasons. To get more insightful results, we remove from `dialogues.tidy` some onomatopoeia.
...
To get more insightful results, when constructing the document term matrix, we only keep the words occurring at least 20 times, and whose tf-idf is higher than the 80% quantile.
...
Let us now allocate the characters to the topic they are most likely to belong to. The following plot shows the gamma probabilities for the most talkative characters. Waylon Smithers is the character with the highest probability of belonging to topic 1. One of the most significant words for topic 1 was 'sir', which is indeed very often used by Smithers to address his boss Mr. Burns! Lisa Simpson, instead, has the highest probability of belonging to topic 2.

...
The Simpsons holds the title of the world's longest-running animated sitcom. It was created by the American writer Matt Groening in 1989, and is nowadays at its 31st season. The show gives us a satirical depiction of the working-class life of the Simpson family, which mainly consists of Homer, Marge, Bart, Lisa, and the little Maggie. 
...
Let us explore these data sets, and see which insights we can get. We start by loading the packages required for the analysis.
...
* `guests` data set: we exclude the Movie season entries, and create a logical variable `self`, which is equal to `TRUE` if a given guest star has played themselves in a particular episode, and `FALSE` if they have instead voiced a regular character. We additionally fix for some typos on the character names.
...
Let us join `dialogues` with the information contained in `characters` and `episodes`. This will allow us to know, for instance, the gender of the characters who talk the most, and the season of each episode. Notice that `dialogues` terminates at episode 568 (episode 16 of season 26).
...
Let us also join `episodes` with the information from `guests`. This will be useful to get, for instance, the guest star names for each episode (if any), and the role they played. Notice that  `episodes` terminates at episode 596 (last episode of season 27), whereas `guests` at episode 662 (episode 23 of season 30). 
...
Lastly, we tidy the `dialogues `, which conveniently puts the speaking lines into a one-word-per-row format.
...
It would be interesting to investigate which characters have the most dialogue within each of these locations. We consider for simplicity the six most recurrent locations.
...
* **Simpson home**: it is the house of the Simpson family. Of course, the most talkative characters here are the Simpson family members.
...
But what is the character that speaks the most throughout the seasons? Let us have a look at the distribution of the total number of words throughout the seasons. We focus on the four most talkative characters, which are Homer, Marge, Lisa and Bart Simpson. Homer Simpson very clearly stands out from any other character.

```{r nb_lines_characters, fig.height=5.8, out.width="580", out.height="580"}
...
dialogues.tidy %>%
  ...
  labs(x = "Number of words per season", y = "Density",
       title = "Total number of words throughout 26 seasons") +
...
Let us have a look at the ratings, the number of votes, and the TV views across episodes and averaged over seasons. We notice an overall downward trend for all three indicators. Older seasons seem to be the most appreciated, most rated, and most viewed. As a matter of fact, the Simpsons show received acclaim throughout its first nine or ten seasons, which are generally considered its "Golden Age", but has been criticized for a perceived [decline in quality](https://en.wikipedia.org/wiki/The_Simpsons#Perception_of_declining_quality) over the years.
...
Let us look at the pairwise-relationships between IMDb ratings, IMDb votes, and TV views. There seems to be positive linear relationships between each pair, suggesting that when one of those indicators increases, so do the other two.
...
The correlation matrix of `episodes` shows indeed a strong inverse relationship between the episode number and the ratings, the votes, and the views. This indicates that more recent seasons are generally affected by a decrease in those performance indicators.
...
The `guests` data set contains information about the guest stars that took part in every episode and their role. Let us have investigate the most recurring guest voices and roles.
...
Let's have a look at the distribution of the number of guest star appearances over time. In the show's early years, most guest stars have voiced original characters, but as the show has continued, the number of those appearing as themselves has increased, especially throughout seasons 12 to 19. In more recent seasons, the gap between the two seem to have become more pronounced.
...
But how many lines are generally reserved to guest stars per episode? Because `dialogues` ends at season 27, we consider `guests` up to season 27. The guest stars with the most lines per episode are the ones voicing a role like a narrator or an announcer and are usually not playing themselves. The only exception of a guest star playing themselves and having a fair amount of lines is Lady Gaga.
...
Most guest stars appeared in a single episode. Guest stars playing themselves tend to have fewer lines than those playing an actual character on the show.
...
Is there a difference in IMDb ratings, IMDb votes, and TV views in the episodes with guest stars playing themselves versus episodes without any guest star? Somehow. It seems that episodes with guests starring themselves have, on average, lower ratings, votes, and views than the episodes with no guest star.
...
The peculiar bigrams were found as the bigrams with the largest tf-idf and that occurred over 50 times. At the top, we find the signature mocking laugh of Nelson 'haw haw', and 'kent brockman' as the TV announcer is always starting off with 'This is Kent Brockman'.
...
Which words - among those occurring at least 400 times - contribute the most to the positive and negative sentiments? Among the positive words, we find e.g., 'love', 'wow', 'nice', and 'fine', whereas among the negative ones 'bad', 'burns', 'stupid', and 'kill'. The word 'burns' is being associated with a negative sentiment because it is seen as coming from the verb 'to burn', which clearly has a negative connotation. In the Simpsons case though, the word 'burn' is likely to just refer to the character called Mr. Burns. However, due to the evil and greedy nature of the character himself, the graph seems pretty accurate after all!
...
The words most responsible of the positive (e.g., 'love', 'nice', 'fine') and negative sentiments  (e.g., 'bad', 'wrong') do not seem to depend that much on the character.
...
Let us now have a look at some wordclouds, which gives us an idea of the most recurrent words throughout the seasons. To get more insightful resuls, we remove from `dialogues.tidy` some onomatopoeia. 
...
We conclude this analysis with some topic modelling. To this end, we need to construct the term document matrix of `dialogues`. It turns out that considering the show episodes as documents does not provide much insight, as episodes tend to share common themes. Therefore, we consider as a document the lines pronounced by a certain character.

To get more insightful resuls, when constructing the document term matrix, we only keep the words occurring at least 20 times, and whose tf-idf is higher than the 80% quantile.
...
Let us perform a Latent Dirichlet Allocation (LDA) analysis on the document term matrix. Let's say we want to find two hidden topics.

```{r lda}
simpsons_lda <- LDA(y, k = 2, control = list(seed = 13))
```

The plot below shows the 10 most representative words for each topic. It seems that the first topic is related to the Simpsons and their social life (as indicated by the words 'moe' and 'beer'), whereas the second one to the Simpsons, their children and family life (as indicated by the words 'dad' and 'homie'). Some words, however, appear on both topics, like 'homer', 'money' and 'god', so the division is not clear-cut.
...
Let us now allocate the characters to the topic they are most likely to belong to. The following plot shows the gamma probabilities for the most talkative characters. Wylon Smithers is the character with the highest probability of belonging to topic 1. One of the most significant word for topic 1 was 'sir', which is indeed very often used by Smithers to address his boss Mr. Burns! Lisa Simpson, instead, has the highest probability of belonging to topic 2.

```{r gamma, fig.height=9, out.width="500"}
